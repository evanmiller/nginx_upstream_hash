...
static char *ngx_http_upstream_hash_merge_conf(ngx_conf_t *cf, void *parent,
    void *child);
...
static ngx_command_t  ngx_http_upstream_hash_commands[] = {
    ...
    { ngx_string("hash_again"),
      ...
      offsetof(ngx_http_upstream_hash_conf_t, retries),
      NULL },
...
static ngx_http_module_t  ngx_http_upstream_hash_module_ctx = {
    ...
    ngx_http_upstream_hash_merge_conf,     /* merge server configuration */
...
ngx_http_upstream_hash_create_conf(ngx_conf_t *cf)
{
    ...
    conf->retries = NGX_CONF_UNSET_UINT;
...
static char *
ngx_http_upstream_hash_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{

    ngx_http_upstream_hash_conf_t  *prev = parent;
    ngx_http_upstream_hash_conf_t  *conf = child;

    ngx_conf_merge_size_value(conf->retries, prev->retries, 0);
